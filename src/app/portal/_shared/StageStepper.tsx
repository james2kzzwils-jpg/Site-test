...
const LABEL_TONE: Record<StageDisplayState, string> = {
approved:         'text-[var(--foreground)]/85',
client_approved:  'text-amber-400',
current:          'text-[var(--foreground)]/85',
past:             'text-[var(--foreground)]/55',
future:           'text-[var(--foreground)]/35',
};
...
return (
  ...
  >
    ...
    <div
      aria-hidden
      className="pointer-events-none absolute left-[10%] right-[10%] top-4 hidden h-px bg-[var(--hairline)] md:block"
    />
    ...
    {STAGE_ORDER.map((kind, idx) => {
      ...
      if (!stage) {
        return (
          ...
          >
            <span
              className="z-10 flex h-8 w-8 items-center justify-center rounded-full border border-[var(--hairline)] bg-[var(--background)] font-mono text-[11px] text-[var(--foreground)]/35"
            >
              {idx + 1}
            </span>
            <span className="mt-2 font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--foreground)]/35">
      ...
      return (
        <div
          key={kind}
          className={`relative flex min-w-[120px] flex-col items-center text-center snap-start
            ${display === 'client_approved' ? 'animate-pulse' : ''}`}
        >
          {/* numbered circle */}
          <span
          ...
          {/* long label */}
          <span className={`mt-2 font-mono text-[10px] uppercase tracking-[0.2em] transition-colors ${LABEL_TONE[display]}`}>
            {STAGE_LONG_LABELS[kind]}
          </span>
